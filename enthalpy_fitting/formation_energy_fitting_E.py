...
electronic equilibria with a mass and electron resevoir, respectively.
Chemical equilibria for NaNbO3-X-Y compounds takes X and Y from the NaNBO3
ternary phase diagram, obtaining chemical_environment[mu][1] and
chemical_environment[mu][0] from chemical_environment[mu][0]NbO3 = mu_X = mu_Y.
Electronic transfer occurs from VBM to a electron reservoir with chemical
potential = donor/aceptor energy level.
The Fermi level in the defected supercell is referenced to system's VBM,
incorporating errors from finite-sized artifacts which are overcome by the
scaling law.
...
height = 80.0
output_file = "fit_formation_energy_"
...
_, bulk_cbm, bulk_vbm, _ = Eigenval(bulk_path + '/EIGENVAL').eigenvalue_band_properties
...
#############################################################################################################################
    ...
    data_O2[..., 1] = data_O2[..., 1] + (+2)*np.array([
                                                    3.2331 - 1.3781,# band 69 - VBM
                                                    3.2717 - 1.3465,# band 573 - VBM
                                                    3.2707 - 1.3196#  band 1941 - VBM
    ...
    with PdfPages(output_file + chemical_environment[mu][2] + ".pdf") as output:
        ...
        axs[0, 0].plot(
            ...
            scaling_law(np.linspace(0, data_Na0[0, 0], num = 100), *final_guess_Na0),
            color = 'black',
            linestyle = 'dashed',
            linewidth = 1.0
        )
        ...
        axs[0, 1].plot(
            ...
            scaling_law(np.linspace(0, data_O0[0, 0], num = 100), *final_guess_O0),
            color = 'black',
            linestyle = 'dashed',
            linewidth = 1.0
        )
        ...
        axs[1, 0].plot(
            ...
            scaling_law(np.linspace(0, data_Na1[0, 0], num = 100), *final_guess_Na1),
            color = 'black',
            linestyle = 'dashed',
            linewidth = 1.0
        )
        ...
        axs[1, 1].plot(
            ...
            scaling_law(np.linspace(0, data_O2[0, 0], num = 100), *final_guess_O2),
            color = 'black',
            linestyle = 'dashed',
            linewidth = 1.0
        )
