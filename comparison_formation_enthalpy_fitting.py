...
height = 80.0
output_file = "comparison_fit_formation_enthalpy"
...
data_Na0_no_correction = data_Na0 + mu_Na
...
data_Na1_no_correction = data_Na1

data_Na1_no_correction[..., 1] = data_Na1_no_correction[..., 1] + (-1)*np.array([
...
data_Na1[..., 1] = data_Na1[..., 1] + (-1)*np.array([
    bulk_vbm + (fermi_level_Na1[0] - vbm_Na1[0]) + 0.000000,
    bulk_vbm + (fermi_level_Na1[1] - vbm_Na1[1]) + 0.394715,
    bulk_vbm + (fermi_level_Na1[2] - vbm_Na1[2]) + 0.413149
]) + mu_Na
...
data_O0_no_correction = data_O0 + mu_O
...
data_O2_no_correction = data_O2
data_O2_no_correction[..., 1] = data_O2[..., 1] + (+2)*np.array([
...
data_O2[..., 1] = data_O2[..., 1] + (+2)*np.array([
    bulk_vbm + (fermi_level_O2[0] - vbm_O2[0]) + 0.153511,
    bulk_vbm + (fermi_level_O2[1] - vbm_O2[1]) + 0.347587,
    bulk_vbm + (fermi_level_O2[2] - vbm_O2[2]) + 0.408538
]) + mu_O
...
(final_guess_Na0_no_correction, pcov) = spop.curve_fit(
    enthalpy,
    data_Na0_no_correction[..., 0],
    data_Na0_no_correction[..., 1],
    initial_guess_Na0
)
...
(final_guess_Na1_no_correction, pcov) = spop.curve_fit(
    enthalpy,
    data_Na1_no_correction[..., 0],
    data_Na1_no_correction[..., 1],
    initial_guess_Na1
)
...
(final_guess_O0_no_correction, pcov) = spop.curve_fit(
    enthalpy,
    data_O0_no_correction[..., 0],
    data_O0_no_correction[..., 1],
    initial_guess_O0
)
...
(final_guess_O2_no_correction, pcov) = spop.curve_fit(
    enthalpy,
    data_O2_no_correction[..., 0],
    data_O2_no_correction[..., 1],
    initial_guess_O2
)

with open(output_file+".txt", "w") as output:
    output.write("With Band Filling and Potential Aligment Corrections\n")
    ...
    output.write("=======================================================================\n")
    output.write("Without Band Filling and Potential Aligment Corrections\n")
    output.write("formation energy fitted with ΔHf (L) = A0 + A1*L⁻¹ + A2*L⁻³\n")
    output.write("v Na 0\n")
    output.write("ΔHf (∞) = A0 = %.4f (%.4f) eV\n" % (final_guess_Na0_no_correction[0], initial_guess_Na0[0]))
    output.write("A1  =  %.4f (%.4f) eV Å\n" % (final_guess_Na0_no_correction[1], initial_guess_Na0[1]))
    output.write("A2  =  %.4f (%.4f) eV Å³\n" % (final_guess_Na0_no_correction[2], initial_guess_Na0[2]))
    output.write("v Na 1-\n")
    output.write("ΔHf (∞) = A0 = %.4f (%.4f) eV\n" % (final_guess_Na1_no_correction[0], initial_guess_Na1[0]))
    output.write("A1  =  %.4f (%.4f) eV Å\n" % (final_guess_Na1_no_correction[1], initial_guess_Na1[1]))
    output.write("A2  =  %.4f (%.4f) eV Å³\n" % (final_guess_Na1_no_correction[2], initial_guess_Na1[2]))
    output.write("v O 0\n")
    output.write("ΔHf (∞) = A0 = %.4f (%.4f) eV\n" % (final_guess_O0_no_correction[0], initial_guess_O0[0]))
    output.write("A1  =  %.4f (%.4f) eV Å\n" % (final_guess_O0_no_correction[1], initial_guess_O0[1]))
    output.write("A2  =  %.4f (%.4f) eV Å³\n" % (final_guess_O0_no_correction[2], initial_guess_O0[2]))
    output.write("v O 2+\n")
    output.write("ΔHf (∞) = A0 = %.4f (%.4f) eV\n" % (final_guess_O2_no_correction[0], initial_guess_O2[0]))
    output.write("A1  =  %.4f (%.4f) eV Å\n" % (final_guess_O2_no_correction[1], initial_guess_O2[1]))
    output.write("A2  =  %.4f (%.4f) eV Å³\n" % (final_guess_O2_no_correction[2], initial_guess_O2[2]))
...
with PdfPages(output_file+".pdf") as output:
    ...
    axs[0, 0].plot(
        np.linspace(0, data_Na0_no_correction[0, 0], num=100),
        enthalpy(np.linspace(0, data_Na0_no_correction[0, 0], num=100), *final_guess_Na0_no_correction),
        color='red',
        linestyle='dashed',
        linewidth=1.0
    )
    ...
    axs[0, 1].plot(
        np.linspace(0, data_O0_no_correction[0, 0], num=100),
        enthalpy(np.linspace(0, data_O0_no_correction[0, 0], num=100), *final_guess_O0_no_correction),
        color='red',
        linestyle='dashed',
        linewidth=1.0
    )
    ...
    axs[1, 0].plot(
        np.linspace(0, data_Na1_no_correction[0, 0], num=100),
        enthalpy(np.linspace(0, data_Na1_no_correction[0, 0], num=100), *final_guess_Na1_no_correction),
        color='red',
        linestyle='dashed',
        linewidth=1.0
    )
    ...
    axs[1, 1].plot(
        np.linspace(0, data_O2_no_correction[0, 0], num=100),
        enthalpy(np.linspace(0, data_O2_no_correction[0, 0], num=100), *final_guess_O2_no_correction),
        color='red',
        linestyle='dashed',
        linewidth=1.0
    )
    ...
    labels = [
        ...
        Line2D(
            ...
            linewidth=1.0,
            linestyle='dashed',
            label=r"Eq.~1.13 w/o corrections"
        )
    ]
    axs[1, 1].legend(
        handles=labels,
